                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td class="p-5 text-center">
                            <?php if($inv['receipt_id']): ?>
                                <div class="inline-flex flex-col items-center">
                                    ...
                                    <a href="receipt_print.php?id=<?= $inv['receipt_id'] ?>" target="_blank" class="mt-1 text-[10px] font-bold text-slate-400 hover:text-blue-600 hover:underline transition-colors">
                                        View Receipt
                                    </a>
                                </div>
                            <?php else: ?>
                                <div class="inline-flex flex-col items-center gap-1.5">
                                    ...
                                    <a href="add_receipt.php?invoice_id=<?= $inv['invoice_id'] ?>&origin=<?= $origin ?>" class="text-[10px] font-bold bg-slate-800 hover:bg-black text-white px-3 py-1 rounded-md shadow-sm transition-transform active:scale-95">
                                        Pay Now
                                    </a>
                                </div>
                            <?php endif; ?>
                        ...
                        <td class="p-5 text-right pr-6">
                            <div class="flex items-center justify-end gap-2">
                                <a href="generate_invoice.php?edit_id=<?= $inv['invoice_id'] ?>" class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-amber-600 hover:border-amber-200 hover:bg-amber-50 transition-all shadow-sm" title="Edit Invoice">
                                    <i class="fa-solid fa-pen-to-square text-xs"></i>
                                </a>

                                <a href="invoice_print.php?id=<?= $inv['invoice_id'] ?>" target="_blank" class="w-8 h-8 inline-flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-200 hover:bg-blue-50 transition-all shadow-sm" title="Print Invoice">
...
$(document).ready(function() {
    var table = $('#invoiceTable').DataTable({
        pageLength: 10,
        // Default sort: none (relies on PHP for initial order)
        order: [], 
        columnDefs: [ 
            { orderable: false, targets: [6] } // Disable sort on Action (Index 6)
        ],
        ...
        drawCallback: function(settings) {
            ...
            // Hide controls if 1 page or less
            if (info.pages <= 1) {
...
function filterType(type, btn) {
    ...
    // Data Filtering - Column 1 is Invoice Number (0 is S.No)
    var table = $('#invoiceTable').DataTable();
