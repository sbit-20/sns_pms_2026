<?php 
// modules/receipts/add_receipt.php
...
// We pass origin so we know where to redirect back to
$origin = isset($_GET['origin']) ? $_GET['origin'] : 'INHOUSE';

$inv = $pdo->query("SELECT * FROM invoices WHERE invoice_id = $iid")->fetch();
if(!$inv) {
    // Update redirect to use BASE_URL
    echo "<script>alert('Invalid Invoice ID'); window.location.href='" . BASE_URL . "dashboard.php';</script>";
    exit;
}
...
<div class="max-w-xl mx-auto mt-10">
    <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <form action="../../src/actions.php" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            <input type="hidden" name="action" value="add_receipt">
            <input type="hidden" name="invoice_id" value="<?= $iid ?>">
            <input type="hidden" name="origin" value="<?= $origin ?>">

            <div class="flutter-input-group">
