...
// Ensure BASE_URL is defined (usually in config.php)
if (!isset($_SESSION['user_id']) && basename($_SERVER['PHP_SELF']) != 'index.php') {
    header("Location: " . BASE_URL . "index.php"); exit;
}

// Helper to check active state based on filename
function isActive($page) {
    return basename($_SERVER['PHP_SELF']) == $page;
}

// Robust logic to check if we are in the Invoice/Receipt module
$current_path = $_SERVER['PHP_SELF'];
$is_invoice_module = (strpos($current_path, 'invoices/') !== false || strpos($current_path, 'receipts/') !== false);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .is-collapsed .nav-label, .is-collapsed .group-label, .is-collapsed .chevron-icon, .is-collapsed .logo-full { display: none !important; opacity: 0; }
    ...
    <aside id="sidebar" class="bg-white hidden md:flex flex-col relative w-64 h-full border-r border-stone-200 z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <div class="h-24 flex items-center justify-center border-b border-stone-100 shrink-0 relative bg-white/50">
            <div class="logo-full items-center justify-center">
                <img src="<?= BASE_URL ?>assets/img/snss.png" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" alt="Logo" class="h-12 w-auto object-contain drop-shadow-sm hover:scale-105 transition-transform">
                <div class="hidden px-4 py-2 bg-stone-800 rounded-lg items-center justify-center text-white font-bold tracking-widest shadow-lg">SUN & SUN</div>
            </div>
            <div class="logo-icon hidden w-full h-full items-center justify-center">
                <div class="w-10 h-10 bg-gradient-to-br from-amber-400 to-amber-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-amber-500/30">S</div>
            </div>
        </div>
        ...
        <div class="p-4 border-t border-stone-100 bg-stone-50/50">
            <a href="<?= BASE_URL ?>logout.php" class="nav-item flex items-center px-3 py-2 text-stone-500 hover:bg-white hover:text-red-600 hover:shadow-sm hover:ring-1 hover:ring-stone-200 rounded-xl transition-all group">
        ...
        <header class="h-16 bg-white/80 backdrop-blur-md border-b border-stone-200/60 flex justify-between items-center px-4 sm:px-8 z-20 sticky top-0">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-2 text-[10px] text-stone-400 font-medium"><span>Overview</span></div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="h-9 w-9 bg-stone-800 rounded-full flex items-center justify-center text-white font-bold text-xs ring-4 ring-stone-100 cursor-pointer hover:ring-amber-100 transition-all">AD</div>
            </div>
        </header>
        ...
        <div id="mobileMenu" class="fixed inset-0 z-50 hidden md:hidden">
            ...
            <div class="absolute left-0 top-0 bottom-0 w-64 bg-white shadow-2xl p-4 flex flex-col gap-2 animate-slide-in">
                <div class="h-20 flex items-center justify-center px-2 mb-4 border-b border-stone-100">
                    <img src="<?= BASE_URL ?>assets/img/snss.png" onerror="this.style.display='none'" alt="Logo" class="h-10 w-auto object-contain">
                </div>
                <a href="<?= BASE_URL ?>dashboard.php" class="p-3 rounded-lg hover:bg-stone-50 font-medium text-stone-600">Dashboard</a>
                <a href="<?= BASE_URL ?>modules/reminders/reminders.php" class="p-3 rounded-lg hover:bg-stone-50 font-medium text-stone-600">Reminders</a>
                <a href="<?= BASE_URL ?>modules/clients/clients.php" class="p-3 rounded-lg hover:bg-stone-50 font-medium text-stone-600">Client Directory</a>
                <a href="<?= BASE_URL ?>modules/invoices/invoice_list.php" class="p-3 rounded-lg hover:bg-stone-50 font-medium text-stone-600">Invoices</a>
                <div class="mt-auto border-t border-stone-100 pt-4">
                    <a href="<?= BASE_URL ?>logout.php" class="p-3 rounded-lg text-red-600 hover:bg-red-50 font-medium flex items-center gap-2"><i class="fa-solid fa-power-off"></i> Sign Out</a>
                </div>
            </div>
        </div>
...
<script>
    ...
    if (savedState === 'collapsed') { sidebar.classList.add('is-collapsed'); icon.classList.add('rotate-180'); }
    ...
    // Persistent Submenu logic
    const sub = document.getElementById('invoiceSubmenu');
    if (sub.classList.contains('hidden') && localStorage.getItem('invoiceSubmenu') === 'open') {
